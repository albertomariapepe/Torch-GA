...
def is_blade_kind(
    blade_degrees: torch.Tensor, kind: Union[BladeKind, str], max_degree: int
) -> torch.Tensor:
    ...
    """
    # Convert kind to string representation for comparison.
    kind = kind.value if isinstance(kind, BladeKind) else kind

    if kind == BladeKind.MV.value:
        return torch.ones(len(blade_degrees), dtype=torch.bool)
...
def invert_blade_indices(num_blades: int, blade_indices: torch.Tensor) -> torch.Tensor:
    ...
    mask = torch.ones(num_blades, dtype=torch.bool)
    mask[blade_indices] = False
    return all_blades[mask]


def get_blade_of_kind_indices(
    blade_degrees: torch.Tensor, kind: BladeKind, max_degree: int, invert: bool = False
) -> torch.Tensor:
    ...
    if invert:
        cond = ~cond
    return torch.nonzero(cond, as_tuple=True)[0]


def _normal_swap(x: List[str]) -> Tuple[bool, List[str]]:
...
def get_blade_indices_from_names(
    ...
    return (
        torch.tensor(blade_signs, dtype=torch.float32),
